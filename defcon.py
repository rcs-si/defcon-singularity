...
from strace_parser import (
    parse_strace_file, summarize_modules, parse_blocked_module_libs,
    parse_conda_roots,
)
...
def stage2(args):
    ...
    conda_roots = _apply_path_overrides(
        parse_conda_roots(trace_path), [], exclude_paths,
    )
    ...
    files = _apply_path_overrides(
        detected_paths=detected_files + conda_roots,
    ...
    if conda_roots:
        ...
        for root in conda_roots:
            print(f"    {root}")
        # Replay activation from a clean state; the pre-job snapshot may describe
        # an unrelated host environment that is absent from the container.
        env_vars = {k: v for k, v in env_vars.items()
                    if not k.startswith("CONDA_") and k not in {
                        "_CE_CONDA", "_CE_M", "_CONDA_EXE", "_CONDA_ROOT",
                    }}
        env_vars["CONDA_ENVS_PATH"] = ":".join(sorted({
    ...
    for root in module_roots:
        ...
        # Conda selects its libraries through activation and package RPATHs.
        if root not in conda_roots:
    ...
    conda_bins = [str(Path(root) / "condabin") for root in conda_roots
                  if (Path(root) / "etc/profile.d/conda.sh").is_file()]
    env_vars["PATH"] = ":".join(conda_bins + module_bins + ["/usr/local/bin"] + base_path_parts)
